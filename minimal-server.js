...
// Proxy route - handle all requests to /:interceptorId/*
app.all('/:interceptorId', async (req, res) => {
  const { interceptorId } = req.params;
  console.log(`Proxy request: ${req.method} /${interceptorId}`);
  ...
  try {
    ...
    console.log(`Proxying to: ${interceptor.base_url}`);
...
// Proxy route for paths after interceptor ID
app.all('/:interceptorId/:path*', async (req, res) => {
  const { interceptorId, path } = req.params;
  console.log(`Proxy request: ${req.method} /${interceptorId}/${path}`);
